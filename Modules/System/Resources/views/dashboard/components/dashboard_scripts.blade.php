<script>
  @can('analytics-widgets')
    ...
    function GetMostVisitedPage(){
        $.ajax({
            type: "GET",
            url: "{{route('dashboard.fetch_most_visited_pages')}}",
            success: function(data)
            {
                $('#most-visited-page').empty();
                $('#most-visited-page').append(data.view);
            },
            error: function(data)
            {
                $('#most-visited-page').empty();
                if(data.status == 400){
                   $('#most-visited-page').append(`<div style="padding:20px;" class="text-center"><h4>Unprocessable entity</h4></div>`)
                } else if(data.status == 403 || data.status == 401){
                    $('#most-visited-page').append(`<div style="padding:20px;" class="text-center"><h4>access Forbidden</h4></div>`)
                }
                else if(data.status == 500){
                    $('#most-visited-page').append(`<div style="padding:20px;" class="text-center"><h4>Server Error</h4></div>`)
                }
            }
        });
    }
    function GetUsersSessionsViewsToday(){
          $.ajax({
            type: "GET",
            url: "{{route('dashboard.fetch_sessions_views_users')}}",
            success: function(data)
            {
                $('#users-today').empty();
                $('#users-today').append(data.users_view);
                $('#page-views-today').empty();
                $('#page-views-today').append(data.page_view);
                $('#sessions-today').empty();
                $('#sessions-today').append(data.sessions_view);
            ...
            error: function(data)
            {
                ...
                $('#users-today').empty();
                if(data.status == 400){
                   $('#sessions-today').append(`<div class="widget-simple-sm-statistic">
                                <div class="number">400</div>
                                <div class="caption color-purple">Unprocessable Entity</div>
                            </div>
                            <div class="widget-simple-sm-bottom statistic">
                                <span class="arrow color-red"></span>Contact Adminstrator<strong></strong>
                            </div>`);
                    $('#users-today').append(`<div class="widget-simple-sm-statistic">
                                <div class="number">400</div>
                                <div class="caption color-purple">Unprocessable Entity</div>
                            </div>
                            <div class="widget-simple-sm-bottom statistic">
                    ...
                    $('#page-views-today').append(`<div class="widget-simple-sm-statistic">
                                <div class="number">400</div>
                                <div class="caption color-purple">Unprocessable Entity</div>
                            </div>
                            <div class="widget-simple-sm-bottom statistic">
                 ...
                 $('#sessions-today').append(`<div class="widget-simple-sm-statistic">
                                ...
                                <div class="caption color-purple">Access Forbidden</div>
                            </div>
                            <div class="widget-simple-sm-bottom statistic">
                                <span class="arrow color-red"></span>Contact Adminstrator<strong></strong>
                            </div>`);
                    ...
                    $('#page-views-today').append(`<div class="widget-simple-sm-statistic">
                         ...
                         $('#sessions-today').append(`<div class="widget-simple-sm-statistic">
                                ...
                                <div class="caption color-purple">Server Error</div>
                            </div>
                            <div class="widget-simple-sm-bottom statistic">
                                <span class="arrow color-red"></span>Contact Adminstrator<strong></strong>
                            </div>`);
    ...
    function GetRealtimeUsers(){
          $.ajax({
            type: "GET",
            url: "{{route('dashboard.fetch_real_time_user')}}",
            success: function(data)
            {
                $('#active-users').empty();
                $('#active-users').append(data.view);
            },
            error: function(data)
            {
                $('#active-users').empty();
                if(data.status == 400){
                   $('#active-users').append(`<div class="widget-simple-sm-statistic">
                                <div class="number">400</div>
                                <div class="caption color-purple">Unprocessable entity</div>
                            </div>
                            <div class="widget-simple-sm-bottom statistic">
    ...
    function GetTopreferrers(){
          $.ajax({
            type: "GET",
            url: "{{route('dashboard.fetch_top_referrers')}}",
            success: function(data)
            {
                $('#top-referrers').empty();
                $('#top-referrers').append(data.view);
                $('[data-toggle="tooltip"]').tooltip();
            },
            error: function(data)
            {
                $('#top-referrers').empty();
                if(data.status == 400){
                   $('#top-referrers').append(`<div style="padding:20px;" class="text-center"><h4>Unprocessable entity</h4></div>`)
                } else if(data.status == 403 || data.status == 401){
                    $('#top-referrers').append(`<div style="padding:20px;" class="text-center"><h4>access Forbidden</h4></div>`)
                }
                else if(data.status == 500){
                    $('#top-referrers').append(`<div style="padding:20px;" class="text-center"><h4>Server Error</h4></div>`)
                }
            }
        });
    }

    $(document).on('click','#refresh_active_users',function(){
        $('.active-refresher-button').addClass('icn-spinner');
        $('.active-numbers').html('<i style="font-size:12px;" class="icn-spinner active-refresher-button panel-control-icon glyphicon glyphicon-refresh"></i>');
          $.ajax({
            type: "GET",
            url: "{{route('dashboard.fetch_real_time_user')}}",
            success: function(data)
...
gapi.analytics.ready(function() {
  ...
  gapi.analytics.auth.authorize({
    ...
    clientid: "{{setting('analytics-client-id')}}"
  });
  ...
  function renderWeekOverWeekChart(ids) {
    ...
    Promise.all([thisWeek, lastWeek]).then(function(results) {
	  ...
	  $('#chart-1-container-loader').remove();
    });
  }
  ...
  function renderYearOverYearChart(ids) {
    ...
    Promise.all([thisYear, lastYear]).then(function(results) {
	  ...
	  $('#chart-2-container-loader').remove();
    })
    .catch(function(err) {
  ...
  function renderTopBrowsersChart(ids) {
    ...
    .then(function(response) {
	  ...
	  $('#chart-3-container-loader').remove();
    });
  }
  ...
  function renderTopCountriesChart(ids) {
    ...
    .then(function(response) {
	  ...
	  $('#chart-4-container-loader').remove();
    });
  }
  ...
  viewSelector.on('change', function(ids) {
    ...
    $('#session-container-loader').remove();
  });
