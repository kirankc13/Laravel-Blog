...
@section('js')
<script>
 $(document).on('click',".delete-users",function(event){
            var id = $(this).attr('cid');
            swal({
                title: "Are you sure?",
                text: "Deleting Users are crucial. Are you you want to Delete this record?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
                })
                .then((willDelete) => {
                if (willDelete) {
                $.post("{{route('users.destroy')}}",{id:id,_token:"{{csrf_token()}}"},function(data){
                    if(data.success == false)
                    {
                         swal("User record cannot be deleted!", {
                        icon: "warning",
                    });
                    }else{
